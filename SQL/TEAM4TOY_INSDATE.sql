...
CREATE OR REPLACE VIEW CABUDGET2
AS
SELECT  MO_MMCODE AS MMCODE,
        ...
        TM_MAX AS TMMAX,
        TM_MIN AS TMMIN,
        TO_CHAR(IO_DATE,'YYMM') AS IODATE,
...
GROUP BY MO_MMCODE, MO_NAME, MO_CODE, MO_TOTAL, TM_MAX, TM_MIN, TO_CHAR(IO_DATE,'YYMM');
...
FROM CABUDGET2 WHERE MMCODE = '1001' AND IODATE = '2208';
...
GROUP BY TMMIN;
/* 내가 지금까지 사용한 총 예산*/
SELECT SUM(SPENDMONEY) AS SPENDTOTALMONEY
FROM CABUDGET2 WHERE MMCODE = '1001' AND IODATE = '2208'
GROUP BY TMMAX;
...
SELECT  (TMMAX - SUM(SPENDMONEY)) AS STATUS,
        TMMAX,
        TMMIN
FROM CABUDGET2 WHERE MMCODE = '1001' AND IODATE = '2208'
